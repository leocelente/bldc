...
void hw_init_gpio(void) {
	...
	palSetPadMode(GPIOC, 4,
			...
			PAL_STM32_OSPEED_HIGHEST); // LED_GREEN
	palSetPadMode(GPIOC, 5,
			PAL_MODE_OUTPUT_PUSHPULL |
			PAL_STM32_OSPEED_HIGHEST); // LED_RED
	...
	// palSetPadMode(GPIOA, 3, PAL_MODE_INPUT_ANALOG); //! SERVO gets configured elsewhere
	palSetPadMode(GPIOA, 4, PAL_MODE_INPUT_ANALOG); // TX_SDA_NSS
	...
	palSetPadMode(GPIOC, 3, PAL_MODE_INPUT_ANALOG); // CURR3
	

	palSetPadMode(GPIOC, 0, PAL_MODE_INPUT_ANALOG); // TEMP_MOTOR
	palSetPadMode(GPIOC, 1, PAL_MODE_INPUT_ANALOG); // ADC_TEMP
...
void hw_setup_adc_channels(void) {
	...
	ADC_RegularChannelConfig(ADC1, ADC_Channel_8, 2, ADC_SampleTime_15Cycles); // CURR2
	ADC_RegularChannelConfig(ADC1, ADC_Channel_Vrefint, 3, ADC_SampleTime_15Cycles); 	// Vrefint
	ADC_RegularChannelConfig(ADC1, ADC_Channel_4, 4, ADC_SampleTime_15Cycles); // TX_SDA_NSS


	ADC_RegularChannelConfig(ADC2, ADC_Channel_1, 1, ADC_SampleTime_15Cycles); // SENS2
	ADC_RegularChannelConfig(ADC2, ADC_Channel_9, 2, ADC_SampleTime_15Cycles); // CURR1
	ADC_RegularChannelConfig(ADC2, ADC_Channel_6, 3, ADC_SampleTime_15Cycles); // ADC_EXT2 
	ADC_RegularChannelConfig(ADC2, ADC_Channel_5, 4, ADC_SampleTime_15Cycles); // ADC_EXT


	ADC_RegularChannelConfig(ADC3, ADC_Channel_2, 1, ADC_SampleTime_15Cycles);  // SENS1
	// ADC_TEMP Changed Channels
	ADC_RegularChannelConfig(ADC3, ADC_Channel_11, 2, ADC_SampleTime_15Cycles); // TEMP_MOSFET (ADC_TEMP)
	...
	/**
	* All the previous settings make sense, they align with exactly one pin and usage. BUT, these InjectedChannels 
	* They have higher priority, so current measurements are more imediate and synchronized to avoid noise. Its is my hypothesis 
	* that they are each configured twice to couple them (sync them) no matter which triggers the event
	 */
	// Injected channels
